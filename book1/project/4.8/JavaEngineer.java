...
class JavaEngineer{
	...
	//删除指定Java工程师资料,实际是把Java工程师相关信息置为初始值
	...
	//查询Java工程师资料，实际是把Java工程师信息资料逐行输出
	public static void searchEngInf(){
		if(status==false){
			System.out.println("Java工程师资料未输入或已删除！");
		}else{
			System.out.println("Java工程师的编号："+engNo);
			System.out.println("Java工程师的姓名："+engName);
			System.out.println("Java工程师的性别："+(engSex==1?"男":"女"));
			switch(engEdu){
				case 1:System.out.println("Java工程师学历：大专");break;
				case 2:System.out.println("Java工程师学历：本科");break;
				case 3:System.out.println("Java工程师学历：硕士");break;
				case 4:System.out.println("Java工程师学历：博士");break;
				case 5:System.out.println("Java工程师学历：其他");break;
				default:System.out.println("Java工程师学历输入不正确");break;
			}
		}
		System.out.println("Java工程师底薪："+basSalary);
		System.out.println("Java工程师月应扣保险数："+insurance);
	}
	//修改Java工程师资料,和输入Java工程师资料功能类似，区别在于需要先输出原信息再让用户输入新修改的信息
	public static void modifyEngInf(){
		if(status==false){
			System.out.print("Java工程师资料未输入或已删除，不能修改！");
		}else{
			status = false;
			while(!status){
				...
				System.out.print("原来Java工程师的性别是："+(engSex==1?"男":"女")+"，请输入修改后的性别：（1代表男，2代表女）");
				engSex = input.nextInt();
				if(engSex!=1&&engSex!=2){
					status = false;
				...
				switch(engEdu){
					case 1:System.out.println("原来Java工程师学历：大专");break;
					case 2:System.out.println("原来Java工程师学历：本科");break;
					case 3:System.out.println("原来Java工程师学历：硕士");break;
					case 4:System.out.println("原来Java工程师学历：博士");break;
					case 5:System.out.println("原来Java工程师学历：其他");break;
					default:System.out.println("原来Java工程师学历不正确！");break;
				}
				System.out.print("请输入修改后的学历：（1代表大专，2代表本科，3代表硕士，4代表博士，5代表其他）");
				engEdu = input.nextInt();
				if(engEdu!=1&&engEdu!=2&&engEdu!=3&&engEdu!=4&&engEdu!=5){
					status = false;
					System.out.println("学历只能输入1、2、3、4、5————（1代表大专，2代表本科，3代表硕士，4代表博士，5代表其他）！");
					continue;
				}else{
					status = true;
				}
				...
				System.out.print("原来Java工程师的月应扣保险金额是："+insurance+",请输入修改后的月应扣保险数：");
				insurance = input.nextDouble();
				if(insurance<=0){
					status = false;
